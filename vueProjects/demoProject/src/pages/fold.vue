...
export default {
  data() {
    return {
      text1:
        "换行换行换行换行换r sad sss行换行行换行换行换换行换行换行换行换行换行行换行换行换行换行换行换行换行换行换",
      text2: "",
    };
  },
  ...
  methods: {
    ...
    handle() {
      ...
      const texts2 = this.handleText(boxWidth - imageWidth, otherText, true);
      const secondLineText = texts2.firstLineText; // 第二行text
      const otherText2 = texts2.otherText; // 剩下的text
      const finalText = firstLineText + "<br>" + secondLineText + otherText2;
